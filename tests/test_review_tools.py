...
class ReviewToolTests(unittest.TestCase):
    ...
    def test_e2e_skill_owns_topic_prompt_without_polluting_writing_skill(self) -> None:
        ...
        self.assertIn("notes/synthesis_model.md", e2e_skill)
        self.assertIn("notes/synthesis_model.md", writing_skill)
        self.assertIn("notes/synthesis_model.md", qoder_adapter)
        self.assertIn("different views of the project's living synthesis model", normalized_figure)
        self.assertIn("there is no separate expansion stage", normalized_writing)
        self.assertNotIn("one substantive expansion pass", e2e_skill)
        self.assertNotIn("one substantive expansion pass", writing_skill)
        self.assertNotIn("one substantive expansion pass", qoder_adapter)
        self.assertNotIn("--include-word-advisory", e2e_skill)
        self.assertIn("only when the user explicitly asks", normalized_writing)
        self.assertIn("before sustained drafting", normalized_figure)
        self.assertIn("not a checklist, completion record, or parallel manuscript", normalized_e2e)
